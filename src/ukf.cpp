...
UKF::UKF() {
  ...
  Xsig_pred_prior = MatrixXd(n_x_, 2*n_aug_ + 1);
...
void UKF::Prediction_Prior(double delta_t){
  //create augmented mean vector
  VectorXd x_aug = VectorXd(n_aug_);
  x_aug.head(5) = x_;
  x_aug(5) = 0;
  ...
  for (int i = 0; i< 2*n_aug_+1; i++)
  {
    ...
    //write predicted sigma point into right column
    Xsig_pred_prior(0,i) = px_p;
    Xsig_pred_prior(1,i) = py_p;
    Xsig_pred_prior(2,i) = v_p;
    Xsig_pred_prior(3,i) = yaw_p;
    Xsig_pred_prior(4,i) = yawd_p;
  }
  ...
  for (int i = 0; i < 2*n_aug_ + 1; ++i){
    ...
    x_prior += weights_(i) * Xsig_pred_prior.col(i);
  }
